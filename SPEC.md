...
- **Win/loss records:** Wrestler cards on the stable selection page, the substitution page's current-stable rows, and its swap-candidate grid show the wrestler's cumulative basho record in sumo notation — a white disc followed by wins, a black disc followed by losses (e.g. ○3 ●2) — right-aligned next to the rank. The discs are CSS-rendered (the pixel font has no circle glyphs). Records count decided bouts only; fusen wins/losses count normally, so a kyujo wrestler's accumulating fusenpai losses are visible when weighing a swap. The record is hidden while a wrestler has no decided bouts (keeping the pre-basho selection window clean). Juryo fill-ins show only their Makuuchi-tracked bouts, not their full Juryo record.
