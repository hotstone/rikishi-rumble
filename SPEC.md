...
- **Active basho determination:** `getActiveBasho()` derives the active basho on every call. It takes the most recent calendar basho start ≤ now (second Sunday of the latest passed odd month), looks up its `start_date` in the `basho` table if present (otherwise uses the calendar date), then: returns the basho if `day` is 1-14; on day 15, returns the basho only if at least one bout still has `winner_id IS NULL` (or no bouts have been synced yet). A running basho is also bounded by the calendar: one grace day after day 15 (allowing a late final sync to backfill), it ends regardless of bout data. When no basho is running, the **next** basho becomes active for selection purposes (status `upcoming`) from **7 days before its start date** — this is the pre-basho window in which admins sync the banzuke and players pick their stables ahead of the day-1 lock. Outside a running basho and the selection window, there is no active basho. No `is_active` flag is stored.
- **Display basho fallback:** `getDisplayBasho()` returns the active basho when there is one; otherwise it falls back to the most recent basho in the `basho` table whose `start_date` has passed, reported with status `completed`. All read-only endpoints (leaderboard, bouts, stables, substitution history, wrestlers, `/api/basho`) use the display basho, so the just-finished basho's results stay visible between tournaments instead of the site going blank. Write endpoints (stable picks, substitutions), sync, and cron still require a genuinely **active** basho, so a completed basho is read-only. The champions list keeps excluding only the active basho, so a completed basho appears there immediately. When the displayed basho is completed, `/api/basho` reports the countdown to the next basho alongside it, and the header subtitle shows "ENDED {day-15 date}" in place of the day counter.
...
- **Win/loss records:** Wrestler cards on the stable selection page (only — the substitution page shows none) display the wrestler's cumulative basho record in sumo notation — a white disc followed by wins, a black disc followed by losses (e.g. ○3 ●2) — right-aligned next to the rank. The discs are CSS-rendered (the pixel font has no circle glyphs). Records count decided bouts only; fusen wins/losses count normally. The record is hidden while a wrestler has no decided bouts (keeping the pre-basho selection window clean). Juryo fill-ins show only their Makuuchi-tracked bouts, not their full Juryo record.
